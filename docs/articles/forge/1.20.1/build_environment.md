...
## 4. 不必要なものを削除する
左側のプロジェクトツリーで、`src\main\java\net\[yourname]\[modid]\`を開き、中に入っている`Config`クラスを削除します。
ウィンドウが表示されますが、Delete Anyway をクリックします。
次に、Modのメインクラスを開き、中身を削って定数`MODID`、コンストラクタのみにしてください。
さらに、コンストラクタの中身も変数`modEventBus`のみにします。